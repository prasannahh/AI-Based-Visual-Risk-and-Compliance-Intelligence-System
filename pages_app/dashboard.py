...
def render():
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f"<div style='text-align:center;color:#22d3ee;font-weight:600;padding-top:0.4rem;'>"
    ...
    with col1:
        with st.container(border=True):
            ...
            if history.empty:
                ...
                fig = go.Figure(
                    go.Scatter(x=history["date"], y=history["cumulative_savings"], mode="lines", line=dict(color="#22d3ee", width=3))
                )
                fig.update_layout(
                    template="plotly_white", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                    font=dict(color="#cbd5e1"),
                    xaxis=dict(color="#cbd5e1", tickfont=dict(color="#cbd5e1")),
                    yaxis=dict(color="#cbd5e1", tickfont=dict(color="#cbd5e1")),
    ...
    with col2:
        with st.container(border=True):
            ...
            if weekly.empty or weekly["hours"].sum() == 0:
                ...
                fig = go.Figure(go.Bar(x=weekly["day"], y=weekly["hours"], marker_color="#22d3ee"))
                fig.update_layout(
                    template="plotly_white", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                    font=dict(color="#cbd5e1"),
                    xaxis=dict(color="#cbd5e1", tickfont=dict(color="#cbd5e1")),
